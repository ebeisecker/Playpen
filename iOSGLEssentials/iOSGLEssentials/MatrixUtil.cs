...
namespace iOSGLEssentials
{
	...
	{
		public static void Multiply(ref float[] ret, float[] lhs, float[] rhs)
		{
			// [ 0 4  8 12 ]   [ 0 4  8 12 ]
			// [ 1 5  9 13 ] x [ 1 5  9 13 ]
			// [ 2 6 10 14 ]   [ 2 6 10 14 ]
		...
		public static void LoadPerspective(ref float[] mtx, float fov, float aspect, float nearZ, float farZ)
		{
			float f = 1.0f / Convert.ToSingle(Math.Tan((fov * (Math.PI / 180.0)) / 2.0f));
		...
		public static void mtxLoadOrthographic(float[] mtx, float left, float right, float bottom, float top, float nearZ, float farZ)
		{
		...
		public static unsafe void mtxModifyObliqueProjection(ref float[] mtx, float[] src, float[] plane)
		{
			var vec = new float[4];
			
			Array.Copy(mtx, src, 16 * sizeof(float));
			...
			var dot = VectorUtil.vec4DotProduct(plane, vec);
		...
		public static void mtxTranspose(ref float[] mtx, float[] src)
		{
...
*/
	public static void LoadIdentity(ref float[] mtx)
	{
		// [ 0 4  8 12 ]
		// [ 1 5  9 13 ]
		// [ 2 6 10 14 ]
	...
	public static void LoadTranslate(ref float[] mtx, float xTrans, float yTrans, float zTrans)
	{
...
*/
	...
	public static void TranslateApply(ref float[] mtx, float xTrans, float yTrans, float zTrans)
	{
		// [ 0 4  8 12 ]   [ 1 0 0 x ]
		// [ 1 5  9 13 ] x [ 0 1 0 y ]
		// [ 2 6 10 14 ]   [ 0 0 1 z ]
	...
	public static void ScaleApply(ref float[] mtx, float xScale, float yScale, float zScale)
	{ 
	    // [ 0 4  8 12 ]   [ x 0 0 0 ]
	    // [ 1 5  9 13 ] x [ 0 y 0 0 ] 
	    // [ 2 6 10 14 ]   [ 0 0 z 0 ]
...
*/
	...
	public static void RotateXApply(ref float[] mtx, float deg)
	{
		// [ 0 4  8 12 ]   [ 1  0    0  0 ]
		...
		float rad = deg * Convert.ToSingle((Math.PI / 180.0f));
		
		float cosrad = Convert.ToSingle(Math.Cos(rad));
		float sinrad = Convert.ToSingle (Math.Sin(rad));
	...
	public static void RotateYApply(ref float[] mtx, float deg)
	{
	...
	public static void RotateZApply(ref float[] mtx, float deg)
	{
	...
	public static void RotateApply(ref float[] mtx, float deg, float xAxis, float yAxis, float zAxis)
	{	
		if(yAxis == 0.0f && zAxis == 0.0f)
		{
			RotateXApply(ref mtx, deg);
		}
		else if(xAxis == 0.0f && zAxis == 0.0f)
		{
			RotateYApply(ref mtx, deg);
		}
		else if(xAxis == 0.0f && yAxis == 0.0f)
		{
			RotateZApply(ref mtx, deg);
		}
		else
		{
			float rad = deg * (float)(Math.PI / 180.0f);
			
			float sin_a = (float)Math.Sin(rad);
			float cos_a = (float)Math.Cos(rad);
			
			// Calculate coeffs.  No need to check for zero magnitude because we wouldn't be here.
			float magnitude = (float)Math.Sqrt(xAxis * xAxis + yAxis * yAxis + zAxis * zAxis);
...
*/
	public static void mtx3x3FromTopLeftOf4x4(ref float[] mtx, float[] src)
	{    
		mtx[0] = src[0];
		mtx[1] = src[1];
		mtx[2] = src[2];
